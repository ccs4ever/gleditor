...
zero naked magic numbers, and data-backed performance across `gleditor`, `apps/xudu`, and
`apps/zigzag`.
...
- **Library Purity**: `include/gleditor/` and `src/` must **never** include or depend on any code in
  `apps/` (`apps/gleditor`, `apps/xudu`, `apps/zigzag`).
- **Application Isolation**: Applications may consume the core library (`include/gleditor/`), but
  must **never** cross-include or depend on each other (`apps/xudu` cannot include `apps/zigzag`,
  and vice-versa).
- **Promotion Discipline**: Reusable utilities, data structures, or algorithms must be moved to
  `src/` rather than copied between apps.
