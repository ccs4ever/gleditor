...
#
# Every invocation binds the same permascroll, because these samples are one
# author's documents and a document holds no primedia of its own: what makes
# xanadoc_b's quotation of xanadoc_a a transclusion rather than a copy is that
# both name the same address in it. This used to be a --dump-permascroll after
# each run and a --permascroll before the next, writing the whole scroll out
# and reading it back a dozen times; the permascroll persists itself now, so
# binding it is the whole of it.
