 ...
 * @brief Implementation of text and binary subspan search and SHA-256 calculation.
 */
...
std::string extractContext(std::string_view text, std::size_t offset,
  ...
  const std::size_t start = (offset > contextPadding) ? (offset - contextPadding) : 0;
...
SourceGrounder::locateTextSubspan(std::string_view parentText,
  ...
  if (exactPos != std::string_view::npos) {
    return SubspanMatch{
        ...
        .mimeType           = "text/plain",
        .sourcePathOrMagnet = "",
        .contextSnippet     = extractContext(parentText, exactPos, quoteSnippet.size()),
    };
  }
...
std::optional<SubspanMatch>
SourceGrounder::locateBinarySubspan(std::span<const std::uint8_t> parentBytes,
...
std::string
SourceGrounder::computeSha256(std::span<const std::uint8_t> bytes) {
...
std::string
SourceGrounder::computeSha256(std::string_view text) {
