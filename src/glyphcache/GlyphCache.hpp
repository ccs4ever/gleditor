...
template <> struct std::hash<FontMapKeyAdapter> {
  std::size_t operator()(const FontMapKeyAdapter &adapter) {
...
private:
  ...
  std::unordered_map<Glib::ustring, std::unordered_map<FontMapKeyAdapter, Pango::Glyph>>
      glyphs;
  ...
  Pango::Glyph addToCache(const Glib::ustring &chr, const FontPtr &font);
...
public:
  ...
  Pango::Glyph put(const Glib::ustring &chr, const FontPtr &font);
};
