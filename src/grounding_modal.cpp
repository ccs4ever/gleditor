...
void GroundingModal::openForQuote(std::string quoteSnippet,
  ...
  const std::string note =
      "All external quotations require a verified parent work in the Xanadu universe. "
      ...
      [quote = std::move(quoteSnippet), onGrounded = std::move(onGrounded),
       onReject](const std::vector<Form::Field> &resFields) {
        ...
        if (sourceTarget.empty()) {
          if (onReject) {
            onReject("External paste rejected: no parent source work was provided");
          }
          return;
        }
        ...
        if (std::filesystem::exists(sourceTarget)) {
          ...
          if (!match) {
            if (onReject) {
              onReject("External paste rejected: quote snippet was not found in source file");
            }
            return;
          }
          if (onGrounded) {
...
void GroundingModal::openForBinary(std::vector<std::uint8_t> excerptBytes,
  ...
  const std::string note =
      "Select the parent media recording/asset from the swarm or specify a local file.";
  ...
  form_.open(
      ...
      [excerpt = std::move(excerptBytes), onGrounded = std::move(onGrounded),
       onReject](const std::vector<Form::Field> &resFields) {
        ...
        if (sourceTarget.empty()) {
          if (onReject) {
            onReject("External media import rejected: no parent source work was provided");
          }
          return;
        }

        if (std::filesystem::exists(sourceTarget)) {
          const auto match = SourceGrounder::groundFileBinary(sourceTarget, excerpt);
          if (!match) {
            if (onReject) {
              onReject("External media import rejected: clip was not found in source file");
            }
            return;
          }
          if (onGrounded) {
      ...
      [onReject]() {
        if (onReject) {
          onReject("External media import rejected: source media grounding required");
        }
      });
}
