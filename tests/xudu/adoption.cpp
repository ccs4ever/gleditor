...
Elsewhere published(const std::uint64_t from   = 100,
  ...
  out.pub = xanadu::publish(theirs, version, out.keys, "essay", "Their Essay", 1,
                          1700000000);
  return out;
}
...
TEST(AdoptionTest, aLinkBetweenTwoPublishedDocumentsCrossesToAThirdMachine) {
  ...
  const auto myPub = xanadu::publish(mine, notes, myKeys, "notes", "My Notes", 1,
...
TEST(AdoptionTest, readingTheSamePublicationTwiceDoesNotDoubleItsLinks) {
  ...
  const auto pub = xanadu::publish(theirs, version, keys, "essay", "Their Essay",
                                 1, 1700000000);
