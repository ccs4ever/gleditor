...
TEST_F(MutableCryptoTest, signingReproducesThePublishedSignature) {
  EXPECT_EQ(xanadu::signMutableItem(
                xanadu::mutableSigningBuffer("", 1, bep44Value), keys)
                .hex(),
            bep44Signature);
  EXPECT_EQ(xanadu::signMutableItem(
...
TEST(MutableLinkTest, parseHexSaysWhyAKeyIsNotOne) {
  ...
  EXPECT_EQ(xanadu::PublicKey::parseHex("abcd").error(), HexError::WrongLength);
  EXPECT_EQ(xanadu::PublicKey::parseHex(std::string(64, 'z')).error(),
            HexError::NonHexDigit);
  const auto key = xanadu::PublicKey::parseHex(std::string(64, 'a'));
  ASSERT_TRUE(key.has_value());
  EXPECT_EQ(key->hex(), std::string(64, 'a'));
  EXPECT_EQ(xanadu::InfoHash::parseHex("12").error(), HexError::WrongLength);
}
