 ...
 * @brief Unit tests for MediaManager staging and compound multi-torrent publication.
 */
...
TEST(CompoundPublicationTest, StageAndSealCompoundTorrents) {
  ...
  xudu::Store store;
  store.insert(xudu::MicroversionId{}, 0, "Quoting diagram from external source.");
  ...
  // 4. Seal Compound
  const auto compound = xudu::sealCompound(
      store, keys, "test_salt", (tempDir / "pub").string(), prov,
      {staged.localPath});
  ...
  EXPECT_TRUE(std::filesystem::exists(tempDir / "pub" / "diagram.png" / "diagram.png"));
  EXPECT_TRUE(std::filesystem::exists(tempDir / "pub" / "diagram.png" / xudu::provenanceFileName));
  EXPECT_TRUE(std::filesystem::exists(tempDir / "pub" / "diagram.png" / xudu::provenanceSigName));

  // 5. Test AuthorCatalog for transcopyright & published body of work tracking
  xudu::AuthorCatalog catalog;
  catalog.recordWork(compound.mainSeal.hash.hex(), compound.mainSeal.provenance);
  ...
  const auto mediaWork = catalog.workForTorrent(compound.mediaTorrents[0].hash.hex());
  ...
  const auto contentLookup = catalog.workForContentHash(mediaWork->contentSha256);
