...
TEST_F(StoreMultiStoreTest, independentStoresForMultipleSourceFiles) {
  ...
  const auto ver1 = store1.insert(xudu::MicroversionId{}, 0, "First document content.");
  ...
  const auto ver2 = store2.insert(xudu::MicroversionId{}, 0, "Second document content from file 2.");
  EXPECT_EQ(ver2.str(), "1"); // Starts cleanly at state 1 in its own ops scroll!
  ...
  const auto ver3 = store3.insert(xudu::MicroversionId{}, 0, "Third document content from file 3.");
  ...
  EXPECT_EQ(store1.rebuild(ver1).materialize(store1), "First document content.");
  EXPECT_EQ(store2.rebuild(ver2).materialize(store2), "Second document content from file 2.");
  EXPECT_EQ(store3.rebuild(ver3).materialize(store3), "Third document content from file 3.");
}
...
TEST_F(StoreMultiStoreTest, preserveTemporaryStoreToPermanentDirectory) {
  ...
  const auto ver = tempStore.insert(xudu::MicroversionId{}, 0, "Notes typed in temporary store.");
