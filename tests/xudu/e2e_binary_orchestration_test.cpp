...
inline const std::string source3Text =
    "Epilogue: Xanadocs transclude, link, and materialize eternal primedia across swarms.";
...
PpmImageInfo inspectPpm(const fs::path &path) {
  ...
  std::vector<char> rawPixels(static_cast<std::size_t>(info.width * info.height * 3));
...
void exportToPng(const fs::path &ppmPath, const fs::path &pngPath) {
  std::string py = "python3 -c \"from PIL import Image; Image.open('" +
                   ppmPath.string() + "').save('" + pngPath.string() + "')\" >/dev/null 2>&1";
...
fs::path findXuduBinary() {
  ...
  for (const auto &cand : candidates) {
    if (fs::exists(cand) && (fs::status(cand).permissions() & fs::perms::owner_exec) != fs::perms::none) {
      return cand;
    }
  }
...
TEST(E2EBinaryOrchestrationTest, fullFiveStepOrchestrationWithVisualVerification) {
  ...
  const auto screenshotDir = fs::current_path() / "build" / "integration_screenshots";
  ...
  EXPECT_GE(info1.distinctColors, 20U) << "Step 1 screenshot has insufficient color detail";
  ...
  const auto s1Hash = InfoHash::fromHex(xudu_test::singleFileHash);
  const auto s1Scroll = Scroll::ofTorrentFile(s1Hash, 0, "fox.txt", 0, 62);
  const auto vA1 = storeA.transcludeExternal(MicroversionId{}, 0, s1Scroll, 0, 62);
  auto pubA = publish(storeA, vA1, authorA, "xanadoc_a", "Alice Study on Fox Behavior", 1, 1700000000, nullptr);
  ...
  const auto s2Hash = InfoHash::fromHex(xudu_test::multiFileHash);
  const auto s2Scroll = Scroll::ofTorrentFile(s2Hash, 0, "one.txt", 0, 27);
  const auto vB1 = storeB.transcludeExternal(MicroversionId{}, 0, s2Scroll, 0, 27);
  auto pubB = publish(storeB, vB1, authorB, "xanadoc_b", "Bob Multi-file Analysis", 1, 1700000100, nullptr);
  ...
  crossDocLink.left.push_back(activeReaderStore.rebuild(verA).spansFor(0, 30).front());
  crossDocLink.right.push_back(activeReaderStore.rebuild(verB).spansFor(0, 20).front());
  ...
  const auto verBTranscluded = activeReaderStore.transclude(verB, 0, verA, 0, 20);
  activeReaderStore.save(storeReader.string());
  ...
  const auto linkPkg1 = publishLinkPackage(curatorKeys, "pkg_curated", "Editorial Curated Commentary",
  ...
  const auto linkPkg2 = publishLinkPackage(curatorKeys, "pkg_external", "Scholarly Epilogue Citations",
                                          1, 1700000300, std::move(pkg2Links), std::move(pkg2Scrolls));

  // Adopt both LinkPackages into activeReaderStore
  const auto adoptRes1 = adoptLinkPackage(activeReaderStore, linkPkg1, ProminenceTier::Curated);
  EXPECT_GT(adoptRes1.linksAdopted, 0U);

  const auto adoptRes2 = adoptLinkPackage(activeReaderStore, linkPkg2, ProminenceTier::Curated);
  ...
  std::cout << "E2E Binary Orchestration completed successfully:\n"
            << "  - Step 1: " << step1Png << " (" << info1.distinctColors << " colors)\n"
            << "  - Step 2: " << step2Png << " (" << info2.distinctColors << " colors)\n"
            << "  - Step 3: " << step3Png << " (" << info3.distinctColors << " colors)\n"
            << "  - Step 4: " << step4Png << " (" << info4.distinctColors << " colors)\n"
            << "  - Step 5: " << step5Png << " (" << info5.distinctColors << " colors)\n";
}
