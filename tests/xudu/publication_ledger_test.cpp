...
TEST(PublicationLedgerTest, MerkleInclusionProofVerification) {
  PublicationLedger ledger;
  const auto e1 = makeSampleEntry("Foundations of Transclusion", "Ted Nelson", 1);
  const auto e2 = makeSampleEntry("Augmenting Human Intellect", "Doug Engelbart", 2);
  const auto e3 = makeSampleEntry("Personal Dynamic Media", "Alan Kay", 3);
  const auto e4 = makeSampleEntry("Computer Lib / Dream Machines", "Ted Nelson", 4);
...
TEST(PublicationLedgerTest, YamlSerializationRoundTrip) {
  ...
  const fs::path tempPath = fs::temp_directory_path() / "test_publication_ledger.yaml";
