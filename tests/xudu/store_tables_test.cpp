...
StoreTables everything() {
  ...
  locked.kind     = xanadu::SegmentKind::Withheld;
...
TEST(StoreTablesTest, aFileThatIsNotOneIsRefusedAndSaysWhy) {
  ...
  try {
    ...
    EXPECT_THAT(
        ...
        testing::HasSubstr("version " +
                           std::to_string(xanadu::storeTablesFormatVersion + 1)));
    EXPECT_THAT(
        std::string{e.what()},
  ...
  // foreign one and still must not be read as an empty store.
  {
    ...
    out.write(reinterpret_cast<const char *>(xanadu::storeTablesSignature.data()),
