 ...
 * @brief Unit tests for SourceGrounder subspan matching and SHA-256
 * computation.
 */
...
TEST(SourceGrounderTest, LocateExactTextSubspan) {
  ...
  const std::string quote =
      "Transclusion is the inclusion of part of a document";
...
TEST(SourceGrounderTest, LocateNormalizedWhitespaceSubspan) {
  ...
  // Quote has different whitespace formatting (single spaces instead of
...
TEST(SourceGrounderTest, LocateBinarySubspan) {
  const std::vector<std::uint8_t> parent  = {0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A,
                                             0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D,
                                             0x49, 0x48, 0x44, 0x52};
  const std::vector<std::uint8_t> excerpt = {0x00, 0x00, 0x00,
                                             0x0D, 0x49, 0x48};
