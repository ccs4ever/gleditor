...
- C++26 library facilities (`function_ref`, `optional<T&>` and optional-as-range, `inplace_vector`,
  two-span `views::concat`, checked `span_at`) come from `<gleditor/cpp26*.hpp>` as
  `gleditor::cpp26::*`, in the library and the apps alike. Never test `__cpp_lib_*` yourself: the
  native-or-fallback choice is made once in `cpp26_select.hpp` so everything linked against one
  `libgleditor` agrees. See `design/cpp26-compatibility.md`.
