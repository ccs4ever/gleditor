...
`gleditor` can be compiled into standalone WebAssembly (Wasm) modules with WebGL 2.0 rendering via
...
1. Ensure git submodules are initialized in the `gleditor` repository:
...
Because modern web browsers restrict loading WebAssembly and assets over `file://` URIs due to CORS
policies, serve the output directory over a local HTTP server:
