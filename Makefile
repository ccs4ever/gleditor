...
SCAN_BUILD_TARGETS      ?= lib gleditor xudu zigzag
SCAN_BUILD_FLAGS        ?= -o $(OBJDIR)/scan-reports
...
# Runs clang-tidy across translation units using compile_commands.json.
# Checks and suppressions are governed by .clang-tidy (kept in sync with
# .clangd by tools/check-config-harmony.sh). Set TIDY_FILES to restrict the
# scan (e.g. TIDY_FILES='src/toast.cpp'), TIDY_CHECKS to override the checks
# list, or TIDY_WARNINGS_AS_ERRORS='*' to enforce a strict zero-warning gate.
tidy: $(OBJDIR)/compile_commands.json
ifdef RUN_CLANG_TIDY
	$(RUN_CLANG_TIDY) -p $(OBJDIR) \
	  -header-filter=$(TIDY_HEADER_FILTER) \
	  $(if $(TIDY_CHECKS),-checks=$(TIDY_CHECKS)) \
	  $(if $(TIDY_WARNINGS_AS_ERRORS),-warnings-as-errors=$(TIDY_WARNINGS_AS_ERRORS)) \
	  -quiet $(TIDY_FILES)
else ifdef CLANG_TIDY
	@echo "run-clang-tidy not found, running clang-tidy..."
	$(GIT_LS) 'src/*.cpp' 'apps/*.cpp' | grep -v '^thirdparty/' | xargs -n 1 $(CLANG_TIDY) -p $(OBJDIR) $(if $(TIDY_CHECKS),-checks=$(TIDY_CHECKS))
else
	@echo "clang-tidy / run-clang-tidy not found, skipping clang-tidy analysis"
endif
.PHONY: tidy

# Runs Clang Static Analyzer via build interception for deep symbolic execution
# of control flow paths, memory safety, and logic errors.
scan-build:
ifdef SCAN_BUILD
	$(SCAN_BUILD) $(SCAN_BUILD_FLAGS) $(MAKE) -j$$(nproc) $(SCAN_BUILD_TARGETS)
else
	@echo "scan-build not found, skipping scan-build static analysis"
endif
.PHONY: scan-build

# Fast backup static analyzer independent of the Clang compiler toolchain.
cppcheck: $(OBJDIR)/compile_commands.json
ifdef CPPCHECK
	$(CPPCHECK) --project=$< $(CPPCHECK_FLAGS)
else
	@echo "cppcheck not found, skipping cppcheck static analysis"
endif
.PHONY: cppcheck

# Runs the two standardized static analyzers: clang-tidy and scan-build.
analyze: tidy scan-build
.PHONY: analyze

# Comprehensive quality gate: checks formatting, text linters, and static analysis.
...
.PHONY: clean doc run test profile shaders install uninstall dist lib \
	...
	check analyze tidy scan-build cppcheck
