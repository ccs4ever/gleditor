 ...
 * @brief Implementation of MediaManager for staged media files and known sources.
 */
...
StagedMedia MediaManager::stageMediaFile(const std::filesystem::path &filePath) {
...
StagedMedia
MediaManager::stageMediaBytes(std::span<const std::uint8_t> bytes,
...
MediaManager::collectKnownSources([[maybe_unused]] const Store *store,
  ...
  if (nullptr != store) {
    ...
    for (const auto &v : allV) {
      ...
      const auto label = text.empty() ? v.str()
                                      : v.str() + ": " + text.substr(0, 30);
      sources.push_back(gleditor::KnownSource{
