...
int runXuduApp(const int argc, char **argv, const bool enableXuzz) {
  gleditor::initLocale();

  const auto state    = std::make_shared<AppState>();
  const bool detailed = wantsEveryOption(argc, argv);

  argparse::ArgumentParser parser(enableXuzz ? "xuzz" : "xudu",
                                  TOSTRING(GLEDITOR_VERSION));
  ...
  try {
    ...
    std::shared_ptr<zigzag::ZigzagVisualizer> zigzagPresentation;
    std::unique_ptr<xudu::BridgeCoordinator> bridgeCoordinator;
    if (enableXuzz) {
      zigzagPresentation =
      ...
      bridgeCoordinator = std::make_unique<xudu::BridgeCoordinator>(
          links, renderer, *state->accessibility);
    ...
    session->setSystemDocChangedCallback(
        [&app, radialMenu, docSwitcher, &pouchDrawer, &links, &map,
         &zigzagPresentation,
         enableXuzz](const xudu::SystemDocKind kind, const xudu::Store &store) {
          ...
          case xudu::SystemDocKind::Layout: {
            ...
            if (enableXuzz && zigzagPresentation) {
