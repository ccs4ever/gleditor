...
char HypertimeGraph::opKindLetter(
    const std::optional<OpKind> kind) noexcept {
  if (!kind.has_value()) {
...
void HypertimeGraph::computeUnobstructedAliasPosition(
  ...
  const std::array<Candidate, 6> candidates = {
      ...
      Candidate{node.x + node.radius * 0.7F + 4.0F, node.y + node.radius + 2.0F}, // Above-Right
      Candidate{node.x + node.radius * 0.7F + 4.0F, node.y - node.radius - bh},    // Below-Right
  ...
  auto testBoxCollision = [&](const float bx, const float by) -> int {
    ...
    for (const auto &other : nodes_) {
      ...
      const float dSq = (other.x - cx) * (other.x - cx) + (other.y - cy) * (other.y - cy);
    ...
    for (const auto &e : allEdges) {
      ...
      auto segIntersectsBox = [&](const float x1, const float y1, const float x2, const float y2) -> bool {
...
void HypertimeGraph::layout(RenderState &, const float screenW,
  ...
  for (std::size_t i = 0; i < nodes_.size(); ++i) {
    ...
    if (!parent.isZero() && nodeIndices.contains(parent)) {
      const auto pIdx = nodeIndices[parent];
  ...
  for (const auto rIdx : rootIndices) {
    nodes_[rIdx].depth = 0;
    nodes_[rIdx].lane = nextLane++;
  }
...
void HypertimeGraph::drawFrame(gleditor::FrameContext &ctx) {
  ...
  canvas_->addLine(panelX_, panelY_, panelX_ + panelW_, panelY_, 1.0F, 0x334155FF);
  canvas_->addLine(panelX_ + panelW_, panelY_, panelX_ + panelW_, panelY_ + panelH_, 1.0F, 0x334155FF);
  canvas_->addLine(panelX_ + panelW_, panelY_ + panelH_, panelX_, panelY_ + panelH_, 1.0F, 0x334155FF);
  canvas_->addLine(panelX_, panelY_ + panelH_, panelX_, panelY_, 1.0F, 0x334155FF);
  ...
  for (std::size_t i = 0; i < nodes_.size(); ++i) {
    ...
    const bool isComp = std::find(comparedVersions_.begin(), comparedVersions_.end(), n.id) != comparedVersions_.end();

    canvas_->setTag(render::tagKindOverlay, kTagNodeBase + static_cast<std::uint32_t>(i));

    if (isCur) {
      drawDisc(*canvas_, n.x, n.y, n.radius + 3.5F, 0x10B98144, 0x10B981FF, 1.5F);
    }
    if (isComp) {
      drawDisc(*canvas_, n.x, n.y, n.radius + (isCur ? 6.0F : 3.5F), 0xFFD70044, 0xFFD700FF, 1.5F);
    }

    const std::uint32_t fillCol = isCur ? 0x064E3BFF : (isComp ? 0x78350FFF : 0x1E293BFF);
    const std::uint32_t borderCol = isCur ? 0x10B981FF : (isComp ? 0xF59E0BFF : 0x64748BFF);
    ...
    const std::uint32_t letterCol = isCur ? 0x6EE7B7FF : (isComp ? 0xFDE68AFF : 0xF1F5F9FF);
    canvas_->addText(ctx.state, n.x - m.width * 0.5F, n.y + m.height * 0.5F - 2.0F, letterStr, letterCol, 0);

    // Unobstructed alias badge
    if (!n.alias.empty()) {
      canvas_->setTag(render::tagKindOverlay, kTagNodeBase + static_cast<std::uint32_t>(i));
      canvas_->addRect(n.aliasX, n.aliasY, n.aliasW, n.aliasH, 0x0F172AEE);
      canvas_->addLine(n.aliasX, n.aliasY, n.aliasX + n.aliasW, n.aliasY, 1.0F, 0x475569FF);
      canvas_->addLine(n.aliasX + n.aliasW, n.aliasY, n.aliasX + n.aliasW, n.aliasY + n.aliasH, 1.0F, 0x475569FF);
      canvas_->addLine(n.aliasX + n.aliasW, n.aliasY + n.aliasH, n.aliasX, n.aliasY + n.aliasH, 1.0F, 0x475569FF);
      canvas_->addLine(n.aliasX, n.aliasY + n.aliasH, n.aliasX, n.aliasY, 1.0F, 0x475569FF);

      canvas_->addText(ctx.state, n.aliasX + 5.0F, n.aliasY + n.aliasH - 4.0F, n.alias, 0xFCD34DFF, 0);
    }
  }
  ...
  canvas_->addRect(scrubberTrackX_, scrubberTrackY_, scrubberTrackW_, trackH, 0x1E293BFF);
  canvas_->addLine(scrubberTrackX_, scrubberTrackY_, scrubberTrackX_ + scrubberTrackW_, scrubberTrackY_, 1.0F, 0x334155FF);
  canvas_->addLine(scrubberTrackX_, scrubberTrackY_ + trackH, scrubberTrackX_ + scrubberTrackW_, scrubberTrackY_ + trackH, 1.0F, 0x334155FF);

  const std::size_t numVers = chronologicalOrder_.size();
  std::size_t currentIdx = 0;
  for (std::size_t i = 0; i < numVers; ++i) {
    const float tx = scrubberTrackX_ + (numVers > 1 ? (static_cast<float>(i) / static_cast<float>(numVers - 1)) * scrubberTrackW_ : scrubberTrackW_ * 0.5F);
    canvas_->addRect(tx - 0.5F, scrubberTrackY_ - 2.0F, 1.0F, trackH + 4.0F, 0x475569FF);
    if (chronologicalOrder_[i] == current_) {
      currentIdx = i;
    }
  }

  scrubberThumbX_ = scrubberTrackX_ + (numVers > 1 ? (static_cast<float>(currentIdx) / static_cast<float>(numVers - 1)) * scrubberTrackW_ : scrubberTrackW_ * 0.5F);
  canvas_->setTag(render::tagKindOverlay, kTagScrubberThumb);
  drawDisc(*canvas_, scrubberThumbX_, scrubberTrackY_ + trackH * 0.5F, 8.0F, 0x10B981FF, 0xFFFFFFFF, 1.5F);

  const std::string scrubLabel = "Time: " + current_.str() + " (" + std::to_string(currentIdx + 1) + "/" + std::to_string(numVers) + ")";
  canvas_->addText(ctx.state, scrubberTrackX_, scrubberTrackY_ + 16.0F, scrubLabel, 0x94A3B8FF, 0);
  ...
  if (comparedVersions_.size() >= 2) {
    ...
    canvas_->addLine(diffX + diffW, diffY, diffX + diffW, diffY + diffH, 1.0F, 0xF59E0BFF);
    canvas_->addLine(diffX + diffW, diffY + diffH, diffX, diffY + diffH, 1.0F, 0xF59E0BFF);
    canvas_->addLine(diffX, diffY + diffH, diffX, diffY, 1.0F, 0xF59E0BFF);

    const std::string diffTitle = "N-WAY DIFF: " + std::to_string(comparedVersions_.size()) + " VERSIONS";
    canvas_->addText(ctx.state, diffX + 8.0F, diffY + diffH - 16.0F, diffTitle, 0xF59E0BFF, 0);
    ...
    const std::string uStr = "Universal (Gold): " + std::to_string(uChars) + " chars";
    const std::string sStr = "Shared (Amber):    " + std::to_string(sChars) + " chars";
    const std::string qStr = "Unique (Mint):     " + std::to_string(qChars) + " chars";
    const std::string dStr = "Limbo (Crimson):   " + std::to_string(dChars) + " chars";

    canvas_->addText(ctx.state, diffX + 8.0F, diffY + diffH - 34.0F, uStr, 0xFFD700FF, 0);
    canvas_->addText(ctx.state, diffX + 8.0F, diffY + diffH - 50.0F, sStr, 0xF59E0BFF, 0);
    canvas_->addText(ctx.state, diffX + 8.0F, diffY + diffH - 66.0F, qStr, 0x10B981FF, 0);
    canvas_->addText(ctx.state, diffX + 8.0F, diffY + diffH - 82.0F, dStr, 0xEF4444FF, 0);

    // Buttons
    canvas_->setTag(render::tagKindOverlay, kTagQuoteButton);
    canvas_->addRect(diffX + 8.0F, diffY + 10.0F, 115.0F, 22.0F, 0x2563EBFF);
    canvas_->addText(ctx.state, diffX + 12.0F, diffY + 26.0F, "Quote into Head", 0xFFFFFFFF, 0);

    canvas_->setTag(render::tagKindOverlay, kTagOpen3DButton);
    canvas_->addRect(diffX + 128.0F, diffY + 10.0F, 82.0F, 22.0F, 0x059669FF);
    canvas_->addText(ctx.state, diffX + 132.0F, diffY + 26.0F, "Open in 3D", 0xFFFFFFFF, 0);

    canvas_->setTag(render::tagKindOverlay, kTagClearComp);
    canvas_->addRect(diffX + 215.0F, diffY + 10.0F, 46.0F, 22.0F, 0xDC2626FF);
    canvas_->addText(ctx.state, diffX + 220.0F, diffY + 26.0F, "Clear", 0xFFFFFFFF, 0);
  }
...
bool HypertimeGraph::picked(const render::PickingResult &pick,
  ...
  if (tag == kTagScrubberThumb || tag == kTagScrubberTrack) {
    ...
    const auto targetIdx = std::min(
        chronologicalOrder_.size() - 1,
        static_cast<std::size_t>(frac * static_cast<float>(chronologicalOrder_.size())));
  ...
  if (tag == kTagQuoteButton) {
    if (quoteHandler_ && !comparedVersions_.empty()) {
      const auto &srcVer = (comparedVersions_.size() > 1 && comparedVersions_[0] == current_)
                               ? comparedVersions_[1]
                               : comparedVersions_[0];
      const auto &st = session_.store(0);
...
void HypertimeGraph::toggleComparison(const MicroversionId &id) {
  const auto it = std::find(comparedVersions_.begin(), comparedVersions_.end(), id);
...
void HypertimeGraph::describe(gleditor::a11y::Builder &into) {
  ...
  for (std::size_t i = 0; i < nodes_.size(); ++i) {
    ...
    std::string label = "Version " + n.id.str() + " [" + std::string(1, n.opLetter) + "]";
    if (!n.alias.empty()) {
      label += " (" + n.alias + ")";
    }
    node.label = label;
  ...
  if (comparedVersions_.size() >= 2) {
    ...
    diffNode.label =
        "Comparative Diff (" + std::to_string(comparedVersions_.size()) +
        " versions)";
  }
}
