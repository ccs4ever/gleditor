...
public:
  ...
  locateTextSubspan(std::string_view parentText,
                    std::string_view quoteSnippet);

  /**
   * @brief Search for a contiguous @p excerptBytes sub-buffer inside @p parentBytes.
   */
  [[nodiscard]] static std::optional<SubspanMatch>
  ...
  [[nodiscard]] static std::string
  computeSha256(std::string_view text);
};
