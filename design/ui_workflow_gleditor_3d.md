...
`apps/gleditor` — elevating the traditional text and document editor into an immersive 3D spatial
environment.
...
Unlike traditional 2D tab bars that clip and hide document state, each document in `gleditor` exists
within its own spatial coordinate slot along a subtle cylindrical orbit in 3D world space:
...
- **Background / Peripheral Documents ($i \neq A$)**:
  ...
  - Relative angular offset: $\Delta \theta_i = (i - A) \cdot \theta_{\text{slot}}$, where
    $\theta_{\text{slot}} = 26^\circ$.
...
1. `Choreograph` timeline interpolates document positions over $T = 0.42\,\text{s}$ using cubic
   ease-in-out.
1. The incoming document glides forward along $+Z$ into focus, rotating into flat alignment with the
   screen.
1. The outgoing document recedes smoothly along $-Z$ into its peripheral slot.
...
The word-processing and operational controls hover directly above the active document quad in 3D
world space.
...
- **Coordinate Anchor**:
  ...
  where $\Delta z_{\text{elevation}} = 12.0\,\text{units}$ keeps the toolbar floating slightly in
  front of the page to eliminate z-fighting and cast a subtle shadow on the document header.
...
- Clicks and hovers are ray-cast from screen space $(x, y)$ through the inverse view-projection
  matrix $\mathbf{M}^{-1}_{\text{VP}}$ into 3D world space.
- Hit testing against toolbar button quads updates visual hover states and dispatches action
  callbacks on mouse down.

### 4.2 Accessibility (`a11y::Source`)

- The floating toolbar is registered with `a11y::Builder` as a `Role::Toolbar` container.
- Each button registers as an accessible `Role::Button` node with accessible name, state, and
