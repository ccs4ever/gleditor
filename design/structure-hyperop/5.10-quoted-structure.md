...
**Status:** ready, with two substantial corrections — §5.10's "traversal falls through at fold time"
...
[§5.5](5.5-extern-refs.md) for persistent boundary references, [§5.6](5.6-compact-binary-v4.md) for
their stable wire form, and [§5.7](5.7-arena-federation.md) for identity-preserving spaces and
explicit edge scopes. **Does not depend on §5.5's production fetch/cache phase** — see §9.
**Provides:** vocabulary adoption to §5.11 and §4.1 preset sharing, which has been promised since
the system-xanadocs note and has never had a mechanism.
...
- **The fold cannot do I/O, and resolution is I/O.** `Manifold::applyStructure()` is `noexcept`
  (`manifold.cpp:186`), takes one node at a time, and [§5.5 §5](5.5-extern-refs.md) already rules
...
attaches the foreign manifold as an explicit-edge-scope space, exposes proxies for the selected
cells, permits only the selected induced edges, and splices that view into the local rank. No
foreign cell or content span is copied; each proxy retains its source-space identity and delegates
...
- **Copying the foreign cells at adopt time.** That is `importManifold`'s *reproducing* half, which
  [§5.9 §2](5.9-anthology-ranks.md) distinguishes from quoting; it loses both the attribution and
...
| selector  | operations                                                                                       | persistent placeholders |
| --------- | ------------------------------------------------------------------------------------------------ | ----------------------- |
...
the mechanism has to pass. Every placeholder into one foreign store shares one scroll cell and its
`d.scroll-refs` membership rank — [§5.5 §2](5.5-extern-refs.md)'s compaction claim, earning its keep
at the first real caller.
...
means the resolver never writes anything, which is what [§5.5](5.5-extern-refs.md) promises
...
and after it are free ([§5.9](5.9-anthology-ranks.md)'s mixed rank), while a local cell *between*
...
/// Where foreign documents come from. A map in tests; the §5.5 phase 4 cache
...
1. **Read the quotation** off `Q`: the root placeholder on `d.quotes`, the selector cell on
   ...
   `ExternOpRef{ScrollId, produces}`; convert it to a `GlobalOpRef` through §5.5's operation pair —
...
1. **Attach the selected view.** Deduplicate or attach the foreign store as a §5.7 space with
   `EdgeScope::Explicit`, ask `proxyFor()` only for the answer cells, and call `permitEdge()` for
   each foreign edge **both** of whose ends are in the answer. Consult the override map by
   `GlobalOpRef`; the splice and permitted edges substitute a local override for that foreign proxy.
   The explicit edge scope is what keeps an ordinary traversal from escaping past the selector.
...
[§5.5 §5](5.5-extern-refs.md) says resolution means loading the foreign store and folding a manifold
...
[§5.9 §3](5.9-anthology-ranks.md) ruled that an anthology pins a state and never tracks a moving
...
chain ([§5.1](5.1-cell-history.md)), so "what was I running last week" is answerable, which the
...
[§5.3](5.3-editions-rank.md) is what would make it expressible: a reference of the shape
...
`(scroll, produces)` and step 4 by `(scroll, produces, selector)`, leaving only federation
attachment, explicit-edge authorization and the splice to redo. Same shape as the bridge's work
package 4 caching, and it should be sized against §7 of the vision, where **no fold-rate measurement
exists** — the honest statement today is that nobody knows what reattaching a settings store costs.

## 9. Phasing

Only phases 4–5 want §5.5's resolver subsystem, and even they do not require it: the `ForeignSource`
...
proceed while §5.5 phase 4 is still unscoped.

1. **The persistent model.** The five dimensions, `Store::quote(...)` taking a `Selector`, and
   `Store::overrideQuotedCell(...)`, over §5.5's `makeExternRef`. Records a quotation, resolves
...
1. **`materialiseQuotations()`** (§5) for the `cell`, `rank` and `closure` selectors against an
   in-memory `ForeignSource`, using §5.7's explicit edge scope. The heart of the section, and the
   point at which a quoted keymap walks. No VQL yet.
...
1. **Production wiring**: `Session` resolving system-store quotations at epoch changes, the states
   on screen, and the "upstream has moved" notice from §7 — the last of which needs §5.5 phase 4's
...
- **`aQuotationPinsItsState`** — advance the foreign store past the quoted state; assert the
  ...
  [§5.9 §3](5.9-anthology-ranks.md)'s ruling, inherited.
...
§5.5's, no fold change, and `walkRank` untouched. Five dimension cells are five ordinary cells.

**The materialiser is where the bugs are**, and the three worth naming are all silent. Omitting an
explicit edge check lets a traversal escape through one uncarried dimension and exposes the whole
foreign store. The splice order (§5 step 7) drops the local rank's tail with no error. And applying
an override after edges have been authorized can leave both the proxy and its local substitute in
the view, which reads as a duplicate rather than as a failure. Each has a test above; write them
before the resolver, not after.
