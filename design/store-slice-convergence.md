...
under 0.08% of an frame time frame, and the same argument `compact_zzcell.hpp`'s own comment already
...
1. ~~**`Resolver` verified-piece cache**, keyed by info hash and piece index. Blocking for the frame
   ...
   **Measured: 4–5x on the shape of a frame.** Sixty short reads scattered over a 1 MiB torrent at
   ...
   distinct pieces. Uncached that is 4.6–5.5 ms of SHA-1, which is most of an frame time frame on its
...
**Two specified designs now want this, from opposite directions, and neither is a scrollbar.**
...
subscript. [Vlog](vlog-logic-extension.md) §6.3 is first-argument clause indexing, which wants a key
rather than an ordinal but breaks on the same fact: a rank answers successor and predecessor and
nothing else. Both propose the same mitigation, an index in a pinned ephemeral island (R8), and both
are honest that it is a mitigation. The experiment above measures whether the *editor* needs an
...
**A candidate answer now exists:**
[`enfilade-rank-indexing.md`](enfilade/enfilade-rank-indexing.md). It takes this section's own
"which is the enfilade, rebuilt" literally — a third replay product of the ops spool, folded beside
`Version` and `Manifold`, with the tree's **Wid** swapped per consumer: a count for VPL's ordinal, a
Bloom summary for Vlog's key, a bounding span set for link discovery. Two things in it are worth
reading even if the proposal is rejected. The scrollbar wants the ordinal→cell map *and its
inverse*, which is what rules out both a cached array and an ordinal stored per cell. And Vlog
cannot sort its clause rank, because clause order is semantic in Prolog — so it needs an index over
an order that must be *preserved*, which is the same requirement append-only-ness already imposes on
the permascroll. **The experiment above is still the next thing to do**; a proposal does not
substitute for it.
...
2.9 µs against an frame time frame, and the falsifiable threshold moves *in the ruling's favour* — a
...
| [`zigzag-multidimensional-space-and-projection.md`](zigzag-multidimensional-space-and-projection.md) | **done** — its banner now records that `Manifold` *is* the model and lists what steps 14–20 changed; §2 is marked superseded, since the struct it describes is no longer stored |
...
| [`zzstructure.hpp`](apps/common/xanadu/zigzag/zzstructure.hpp) doc comments                          | **done** for `Preflet` (step 18, with the reasoning for why the locator was a layering violation); `d.dims` and `d.meta-dims` are still unnamed there                           |
...
| [`CLAUDE.md`](CLAUDE.md)'s `compact_zzcell.hpp` bullet                                               | **done** — 792 bytes measured, described as the model `Manifold` replaced rather than as the model, and the promised hot/cold split replaced by what actually happened          |
| [`README.md`](README.md)'s engine section                                                            | **done** — `Manifold` as the model, the lock-free permascroll read path, the verified-piece cache, scalars, and `sliceToStore()`                                                |
| [`permascroll-holes-and-transcopyright.md`](permascroll-holes-and-transcopyright.md)                 | **done** — resolution status and transcopyright live in `ColdCell` beside the manifold, not in a cell                                                                           |
| [`audit-2026-09-01-three-day-review.md`](audit-2026-09-01-three-day-review.md)                       | **done** — §4.1 carries a note that its finding was resolved by replacing the struct rather than shrinking it; the audit is otherwise left as the snapshot it is                |
